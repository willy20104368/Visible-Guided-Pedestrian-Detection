...
def test(data,
         ...
         MR2=False,  # for log average miss rate
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save-mr', action='store_true', help='save results to *.txt')
    ...
    parser.add_argument('--deploy', action='store_true', help='run model after reparameterization ')
    ...
    if(opt.deploy):
        ...
        exit()
    print("testing model from {}".format(os.path.join(opt.weights[0],'best.pt')))
    if opt.best:
        best_MR = test(opt.data,
                    os.path.join(opt.weights[0], "best.pt"),
                    opt.batch_size,
                    opt.img_size,
                    opt.conf_thres,
                    opt.iou_thres,
    ...
    best_epoch = opt.weights[0] + "best.pt"
    for w in glob.glob(os.path.join(opt.weights[0], "best_*.pt")):
        if opt.task in ('train', 'val', 'test'):  # run normally
            print("testing model from {}".format(w))
            mean_MR = test(opt.data,
                w,
                opt.batch_size,
                opt.img_size,
                opt.conf_thres,
            ...
            if mean_MR[0] < best_MR[0]:
                best_MR = mean_MR
                best_epoch = w
            print("best_MR:", best_MR[0])
