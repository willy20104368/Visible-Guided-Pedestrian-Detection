...
class Model(nn.Module):
    def __init__(self, cfg='yolor-csp-c.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, IKeypoint):
            ...
            # print('Strides: %s' % m.stride.tolist())
        if isinstance(m, IV6Detect) or isinstance(m, AuxDetect_AF) or isinstance(m ,AuxDetect_AF_e2e) or isinstance(m, AfDetect):
            s = 256  # 2x min stride
    ...
    def forward_once(self, x, profile=False):
        ...
        for m in self.model:
            ...
            if profile:
                c = isinstance(m, (Detect, IDetect, IAuxDetect, IBin)) or isinstance(m, (IAuxDetect_triple, IAuxDetect_vis, IV6Detect, AuxDetect_AF, AuxDetect_AF_e2e, AfDetect))
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        elif m in [IV6Detect, AuxDetect_AF, AuxDetect_AF_e2e, AfDetect]:
            args.append([ch[x] for x in f])
        elif m is ReOrg:
            c2 = ch[f] * 4
        elif m is Contract:
