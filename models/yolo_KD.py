...
class Model(nn.Module):
    def __init__(self, cfg='yolor-csp-c.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, IAuxDetect):
            ...
            # print('Strides: %s' % m.stride.tolist())
        if isinstance(m, IAuxDetect_Map):
            s = 256  # 2x min stride
        ...
        if isinstance(m, IV6Detect) or isinstance(m, AuxDetect_AF) or isinstance(m, AfDetect):
            s = 256  # 2x min stride
            forward = lambda x: self.forward(x)[0] if isinstance(m, IV6Detect) or isinstance(m, AuxDetect_AF)\
                    or isinstance(m, AfDetect) else self.forward(x)
    ...
    def forward(self, x, augment=False, profile=False, feature_layers=None):
        if augment:
        ...
        elif feature_layers != None:
            preds, features = self.forward_once(x, profile, feature_layers)
            return preds, features
        else:
            return self.forward_once(x, profile, feature_layers)  # single-scale inference, train

    def forward_once(self, x, profile=False, feature_layers=None):
        y, dt = [], []  # outputs
        features = []
        for idx, m in enumerate(self.model):
            ...
            if self.traced:
                if isinstance(m, Detect) or isinstance(m, IDetect) or isinstance(m, IAuxDetect) or isinstance(m, IKeypoint) or isinstance(m, Decoupled_Detect) or isinstance(m, IAuxDetect_Map):
                    break

            if profile:
                c = isinstance(m, (Detect, IDetect, IAuxDetect, IBin)) or isinstance(m, (Decoupled_Detect, IAuxDetect_Map))
            ...
            x = m(x)  # run
            if feature_layers and idx in feature_layers:
                features.append(x)
                # print(idx, m)
    ...
    def fuse(self):  # fuse model Conv2d() + BatchNorm2d() layers
        ...
        for m in self.model.modules():
            if isinstance(m, RepConv):
                ...
                delattr(m, 'bn')  # remove batchnorm
                m.forward = m.fuseforward  # update forward
            elif isinstance(m, (IDetect, IAuxDetect, IAuxDetect_Map)):
                m.fuse()
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [nn.Conv2d, Conv, Conv_LN, RobustConv, RobustConv2, DWConv, GhostConv, RepConv, RepConv_OREPA, DownC, 
            ...
            c2 = ch[f] // 2
        elif m in [Detect, IDetect, IAuxDetect, IAuxDetect_Map, IBin, IKeypoint]:
            ...
            if isinstance(args[1], int):  # number of anchors
                args[1] = [list(range(args[1] * 2))] * len(f)
        elif m in [IV6Detect, AuxDetect_AF, AfDetect]:
            args.append([ch[x] for x in f])
        elif m is ReOrg:
            c2 = ch[f] * 4
        elif m is Contract:
