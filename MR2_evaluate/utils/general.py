...
# def xywhn2xyxy_v2(x, w=640, h=640, padw=0, padh=0):
#     # Convert nx4 boxes from [x, y, w, h] normalized to [x1, y1, x2, y2] where xy1=top-left, xy2=bottom-right
#     y = x.clone() if isinstance(x, torch.Tensor) else np.copy(x)
#     y[:, 0] = w * (x[:, 0] - x[:, 2] / 2) + padw  # top left x
#     y[:, 1] = h * (x[:, 1] - x[:, 3] / 2) + padh  # top left y
...
def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False, EIoU=False, SIoU=False, TIoU=False, eps=1e-7):
    # Returns the IoU of box1 to box2. box1 is 4, box2 is nx4
    box2 = box2.T
    ...
    if CIoU or DIoU or GIoU or EIoU or SIoU:
        ...
        if CIoU or DIoU or EIoU or SIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if DIoU: #DIoU
                ...
                cos = torch.nn.CosineSimilarity(dim=1, eps=eps)((b1_x2-b1_x1,b1_y2-b1_y1),(b2_x2-b2_x1,b2_y2-b2_y1))
                return iou-(rho2/c2 + w_dis/cw2 + h_dis/ch2 + cos)
...
def bbox_iou_v6(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, eps=1e-7):
...
def attr_diff(attr1, attr2):
    similarity = F.cosine_similarity(attr1, attr2, dim=1)
    return similarity
...
def box_iou_v6(box1, box2, eps=1e-7):
    # https://github.com/pytorch/vision/blob/master/torchvision/ops/boxes.py
    """
    Return intersection-over-union (Jaccard index) of boxes.
    Both sets of boxes are expected to be in (x1, y1, x2, y2) format.
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    # print(f'Max number of NMS: {max_nms}')
    t = time.time()
    output = [torch.zeros((0, 6), device=prediction.device)] * prediction.shape[0]
    for xi, x in enumerate(prediction):  # image index, image inference
        # Apply constraints
...
# for anchor free detector
def non_max_suppression_v6(
        prediction,
        conf_thres=0.25,
        iou_thres=0.45,
        classes=None,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        box = xywh2xyxy_v6(box)  # center_x, center_y, width, height) to (x1, y1, x2, y2)
        if multi_label:
        ...
        if merge and (1 < n < 3E3):  # Merge NMS (boxes merged using weighted mean)
            # update boxes as boxes(i,4) = weights(i,n) * boxes(n,4)
            iou = box_iou_v6(boxes[i], boxes) > iou_thres  # iou matrix
            weights = iou * scores[None]  # box weights
            x[i, :4] = torch.mm(weights, x[:, :4]).float() / weights.sum(1, keepdim=True)  # merged boxes
            if redundant:
                i = i[iou.sum(1) > 1]  # require redundancy
...
def end2end_postprocessing(prediction,
        conf_thres=0.25,
        classes=None,
        multi_label=False,
        labels=(),
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if not n:  # no boxes
            ...
            x = x[x[:, 4].argsort(descending=True)]  # sort by confidence
        output[xi] = x if x.shape[0] < max_det else x[:max_det]
    return output 

def attr_nms(prediction, attr, conf_thres=0.25, iou_thres=0.45, attr_thres=0.95, classes=None, agnostic=False, multi_label=False,
                        labels=()):
    """Runs Non-Maximum Suppression (NMS) on inference results
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if not n:  # no boxes
            ...
            mask = x[:, 4].argsort(descending=True)[:max_nms]
            x = x[mask]  # sort by confidence
            a = a[mask]
        else:
            mask = x[:, 4].argsort(descending=True)
            x = x[mask]
            a = a[mask]
        # Batched NMS
        c = x[:, 5:6] * (0 if agnostic else max_wh)  # classes
        ...
        while len(boxes):
            ...
            if len(boxes) == 1:
                break
            attr_sim = attr_diff(a[0], a[1:])
            iou = bbox_iou(boxes[0], boxes[1:])
            boxes = boxes[1:]
            a = a[1:]
            x = x[1:]
            mask1 = iou <= iou_thres
            mask2 = attr_sim <= attr_thres
            mask = mask1 & mask2
            # print(a[~mask2].size())
            boxes = boxes[mask]
            a = a[mask]
