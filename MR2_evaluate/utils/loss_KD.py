...
from utils.loss_rep import repulsion_loss_torch, repbox_loss, IoG
...
class ComputeLossAuxOTA_rep_kd:
    # Compute losses
    def __init__(self, model, autobalance=False):
        super(ComputeLossAuxOTA_rep_kd, self).__init__()
    ...
    def __call__(self, p, targets, imgs, features_t=None, features_s=None, pred_t = None):  # predictions, targets, model   
        device = targets.device
        lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        lrepBox, lrepGT = torch.zeros(1, device=device), torch.zeros(1, device=device)
        lfeat = torch.zeros(1, device=device)
        ...
        # feature-base KD loss
        for i in range(len(features_s)):
            lfeat += nn.MSELoss()(features_s[i],  features_t[i])
        lfeat *= 0.03
        ...
        loss = lbox + lobj + lcls + lrep + lfeat
        return loss * bs, torch.cat((lbox, lobj, lrep, lfeat, loss)).detach()


    def build_targets_teacher(self, p_t, targets):
        pass
