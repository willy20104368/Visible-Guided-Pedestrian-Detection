...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            # Load mosaic
            if random.random() < 0.8: # default 0.8
            ...
            if labels.size:  # normalized xywh to pixel xyxy format
                ...
                labels[:, 5:] = xywhn2xyxy(labels[:, 5:], ratio[0] * w, ratio[1] * h, padw=pad[0], padh=pad[1])
            ...
            if not mosaic:
                ...
                if random.random() < hyp['mixup']:
                    ...
                    if labels2.size:  # normalized xywh to pixel xyxy format
                        ...
                        labels2[:, 5:] = xywhn2xyxy(labels2[:, 5:], ratio2[0] * w2, ratio2[1] * h2, padw=pad2[0], padh=pad2[1])
...
def copy_paste(img, labels, segments, probability=0.5):
    ...
    if probability and n:
        ...
        for j in random.sample(range(n), k=round(probability * n)):
            ...
            ioa_aux = bbox_ioa(box_aux, label[:, 5:9])
