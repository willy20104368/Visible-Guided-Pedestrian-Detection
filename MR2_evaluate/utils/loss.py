...
#                             [1, 0], [0, 1], [-1, 0], [0, -1],  # j,k,l,m(左 上 右 下)
...
#             # t是gt经过一定的处理，得到的变量  t.shape：(3, m, 2)
#             # anchors记录当前特征图对应的anchor size
...
#                 # 默认self.hyp['anchor_t']=4，代表target宽高与anchor宽高比例都必须处于1/4到4区间内，才能与当前anchor匹配
#                 j = torch.max(r, 1. / r).max(2)[0] < self.hyp['anchor_t']  # compare
#                 # j = wh_iou(anchors, t[:, 4:6]) > model.hyp['iou_t']  # iou(3,n)=wh_iou(anchors(3,2), gwh(n,2))
#                 # t代表保留的gt
#                 # 到这里后只保留了能与当前特征图的3个anchors匹配的gt
#                 t = t[j]  # filter

#                 # Offsets
#                 gxy = t[:, 2:4]  # grid xy 以特征图左上角为原点，gt的xy坐标
...
class ComputeLossOTA_Rep:
    # Compute losses
    def __init__(self, model, autobalance=False):
        super(ComputeLossOTA_Rep, self).__init__()
        device = next(model.parameters()).device  # get model device
        h = model.hyp  # hyperparameters
